...
const ChatContextMenu = (props: Props) => {
  ...
  return (
    <Motion.div
      ...
      ref={ref!}
      class="absolute z-50 h-max w-52 rounded-xl bg-base-100 p-1"
      classList={{
    ...
    >
      <div>
        ...
        <Show when={props.message().is_read && props.self()}>
          <div class="mt-1 flex w-full items-center gap-4 border-t-2 border-neutral-300 px-3 py-1.5 pt-2 text-start text-neutral-100">
            <Tick
              variant="double"
              class="size-4"
            />
